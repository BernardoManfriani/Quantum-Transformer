...
@cudaq.kernel
def unitary(
    qubits: cudaq.qview, theta: list[float], subsystem_size: int, func: int, layers: int
):
    """
    ...
    for layer in range(layers):
        ...
        for i in range(start, end):
            ry(theta[i - start + layer * (end - start)], qubits[i])  # type: ignore  # noqa: F821
...
def controlled_adjoint_unitary(
    ...
    for layer in range(layers - 1, -1, -1):
        ...
        for i in range(end - 1, start - 1, -1):
            ...
            ry.ctrl(-theta[angle_index], control, qubits[i])  # type: ignore  # noqa: F821
...
def controlled_unitary(
    ...
    for layer in range(layers):
        # Apply the first set of rotations for the initial layer
        for i in range(start, end):
            ry.ctrl(  # type: ignore  # noqa: F821
                theta[i - start + layer * (end - start)], control, qubits[i]
            )
